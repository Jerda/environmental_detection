...
use App\Libraries\WechatTool\Material\Material;
...
//            $this->userToSession($message['FromUserName']);
...
//                return 12;
    ...
    {
        $user = User::with('farmer')->with('worker')
            ->with('wechat')->whereHas('wechat', function ($query) use ($openid) {
    ...
    {
        $user = $this->getUserByOpenID($this->app->oauth->user()->getId());
